...
#-----------------------------------------------------------------------------#
#Walk into ./output_src/ROI/images directory
for dir_path, dir_names, image_names in os.walk( os.path.join("output_src", "ROI", "images") ):
    
    #-------------------------------------------------------------------------#
    #Create (if no exist) the directories to store the HOGvectorsPerImg
    
    #Set HOG directory name as current dir_path
    fileHOGpath = os.path.join("output_src", "HOG", "{0}".format(dir_path[27:]))
    ...
    with h5py.File(os.path.join("output_src", "HOG", "HOGshapeAllImgs.hdf5"), "a") as f:
        pass
    #-------------------------------------------------------------------------#
